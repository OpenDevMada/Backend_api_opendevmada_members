...
Base URL (exemple):
```
http://localhost/api/opendevmada
```
...
## 1) Connexion membre
- URL complète: `http://localhost/api/opendevmada/membre-login`
- Méthode HTTP: `POST`
- Headers requis:
...
curl -X POST \
  ...
  http://localhost/api/opendevmada/membre-login
```
...
- URL complète: `http://localhost/api/opendevmada/membre-logout/{id}`
- Méthode HTTP: `POST`
- Paramètres URL:
...
- Exemple CURL:
```bash
curl -X POST http://localhost/api/opendevmada/membre-logout/12
```
- Exemple de réponse (200):
```json
{
...
- URL complète: `http://localhost/api/opendevmada/membres`
- Méthode HTTP: `GET`
- Paramètres: aucun
- Exemple CURL:
```bash
curl http://localhost/api/opendevmada/membres
```
...
- URL complète: `http://localhost/api/opendevmada/membre/{id}`
- Méthode HTTP: `GET`
- Paramètres URL:
  - `id` (integer)
- Exemple CURL:
```bash
curl http://localhost/api/opendevmada/membre/5
```
- Exemple de réponse (200):
```json
{
...
## 5) Créer un membre
- URL complète: `http://localhost/api/opendevmada/membre-create`
- Méthode HTTP: `POST`
...
- Exemple CURL:
```bash
curl -X POST http://localhost/api/opendevmada/membre-create \
  -F nom="Doe" \
  -F prenom="John" \
...
- URL complète: `http://localhost/api/opendevmada/membre-update/{id}`
- Méthode HTTP: `POST`
- Headers: `Content-Type: multipart/form-data`
- Paramètres URL:
  - `id` (integer)
...
- Exemple CURL:
```bash
curl -X POST http://localhost/api/opendevmada/membre-update/5 \
...
- URL complète: `http://localhost/api/opendevmada/membre-delete/{id}`
- Méthode HTTP: `DELETE`
- Paramètres URL:
  - `id` (integer)
- Exemple CURL:
```bash
curl -X DELETE http://localhost/api/opendevmada/membre-delete/5
```
- Exemple de réponse (200):
```json
{
...
## En-têtes CORS
Les en-têtes suivants sont renvoyés pour toutes les routes:
- `Access-Control-Allow-Origin: *`
- `Access-Control-Allow-Methods: GET, POST, PUT, DELETE, OPTIONS`
- `Access-Control-Allow-Headers: Content-Type`

Note: les requêtes `OPTIONS` (prévol) sont autorisées par les en-têtes, mais il n'y a pas de gestion spécifique de la méthode `OPTIONS` dans les routes.
